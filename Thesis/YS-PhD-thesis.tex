...
\usepackage{float, graphicx, multirow,parskip, subcaption}
\usepackage{comment}
\usepackage{url}
\usepackage{enumitem}
\usepackage{tikz}
...
%\addbibresource{refs.bib}%% <== change to YOUR bib file <= CHANGE
...
\title{Forecasting for Variant Discovery and Latent Dynamics in Genomics}
...
\Author{Yunyi Shen}{Department of Electrical Engineering and Computer Science}[S.M. Computer Science, MIT, 2024][M.S. Statistics, University of Wisconsin--Madison, 2021][M.S. Wildlife Ecology, University of Wisconsin--Madison, 2021][B.S. Biology, Peking University, 2018]
...
\Supervisor{Tamara Broderick}{Associate Professor of Computer Science}
...
\begin{abstract}
	\input{connections/abstract.tex}% use \input rather than \include because we're inside an environment
\end{abstract}

%\input{tex/acknowledgement.tex}% .tex extension is presumed by \include 
...
{
\input{Ch1-intro/ch1.tex}
}


\chapter{Forecasting variant discovery in population genetics surveys}
\label{ch:doubletrouble}

{
\input{Ch2-double-trouble.tex}
}


\chapter{Inferring latent trajectories from snapshots}
\label{ch:sbirr}
...
\chapter{Forecasting stochastic dynamic systems from snapshots}
\label{ch:snapmmd}

{
\input{Ch4-snapmmd.tex}
}
...
{
\input{Ch5-conclusion/conclusion.tex}
}
